...
app.post("/submit", (req, res) => {
  const randomAdjSelector = Math.floor(Math.random() * (adj.length));
  const randomAdj = adj[randomAdjSelector];

  const randomNounSelector = Math.floor(Math.random() * (noun.length));
  const randomNoun = noun[randomNounSelector];

  const randomBandName = randomAdj + " " + randomNoun ;

  res.render("index.ejs",
   {randomBand : randomBandName});
